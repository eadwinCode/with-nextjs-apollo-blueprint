...
type WithApolloProps<TCacheShape> = {
 ...
 */
export const withApollo = ({ ssr = false } = {}) => (PageComponent: React.FC) => {
  const WithApollo: React.FunctionComponent<WithApolloProps<any>> = ({ apolloClient, apolloState, ...pageProps }) => {
    if (apolloClient) {
      // Happens on: getDataFromTree & next.js ssr
      globalApolloClient = apolloClient
    } else {
      // Happens on: next.js csr
      globalApolloClient = initApolloClient(apolloState, undefined)
    }

    return (
      <ApolloProvider client={globalApolloClient}>
  ...
  if (ssr || (PageComponent as any).getInitialProps) {
    (WithApollo as any).getInitialProps = async (ctx:any) => {
      const inAppContext = Boolean(ctx.ctx)
